...
class MilvusDbAdapter(IDbClient):
    def __init__(self, database_name =None, host=None, port=None, user=None, password=None , collection_name=None):
        ...
        self.db_name = database_name if database_name is not None else os.getenv('API_DB_NAME_', 'default')
        ...
        self.password = password if password is not None else os.getenv('API_DB_PASSWORD', '')
        self.collection_name = collection_name if collection_name is not None else os.getenv('COLLECTION_NAME', 'default')

        try:
            self.client = connections.connect(host=self.host, 
                                            port=self.port, 
                                            user=self.user, 
    ...
    def _create_collection(self): 
        documents = FieldSchema(name="documents", dtype=DataType.STRING, is_primary=True, auto_id=False)
        ...
        schema = CollectionSchema(fields=[documents, embeddings, ids, metadatas], primary_field="documents", description="RAG collection")
        collection = Collection(name=self.collection_name, schema=schema)

        return collection
    
    def add_document(self, document, embedding, block : Block):
        self.collection.insert([[document], [embedding], [block.index], [block.to_dict()]])
    """
    ...
    def update_document(self, document, embedding, block: Block):
        self.collection.upsert([[document], [embedding], [block.index], [block.to_dict()]])
