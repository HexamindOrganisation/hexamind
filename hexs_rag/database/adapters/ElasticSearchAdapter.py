...
class ElasticSearchAdapter(IDbClient):
    def __init__(self,host=None, port=None, cloud_id=None, api_key=None,ca_certs=None, ssl_assert=None, password=None):
        ...
        if self.env_params['cloud_id'] is not None and self.env_params['api_key'] is not None:
            self.client = Elasticsearch(
                cloud_id=self.env_params['cloud_id'],
                api_key=self.env_params['api_key']
            )
        else:
            connection_opts = {
            ...
            if self.env_params['ca_certs'] is not None and  self.env_params['ssl_assert'] is not None:
                raise ValueError("You can't use both ca_certs and ssl_assert at the same time")
            elif self.env_params['ssl_assert'] is not None and self.env_params['ca_certs'] is None:
                connection_opts['ssl_assert_fingerprint'] = self.env_params['ssl_assert']
            elif self.env_params['ca_certs'] is not None and self.env_params['ssl_assert'] is None:
    ...
    def add_document(self, document, embedding, block):
        self.client.index(index="rag", document={"document": document, "embedding": embedding, "id":  block.index, "block": block.to_dict()})
