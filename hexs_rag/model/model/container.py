...
class Container:
    ...
    def __init__(self, paragraphs: List[Paragraph], title: Optional[Paragraph] = None, level: int = 0, index: Optional[List[int]] = None, father: Optional['Container'] = None, id_: int = 0) -> None:
        self.level = level
        self.title = title
        ...
        if paragraphs:
            self.paragraphs, self.children = self.create_children(paragraphs, level, self.index)
        
        self.containers: List[Container] = [self] + [child for child in self.children for container in child.containers]
    ...
    def _classify_paragraphs(self, paragraphs: List[Paragraph]) -> (List[Paragraph], List[Paragraph], bool, int):
        ...
        for paragraph in paragraphs:
            if not in_children and not paragraph.is_structure:
                ...
                if paragraph.is_structure and paragraph.level <= current_level:
                    return attached_paragraphs, container_paragraphs, True, paragraph.level
                else:
                    container_paragraphs.append(paragraph)
        return attached_paragraphs, container_paragraphs, in_children, current_level

    def _create_child_container(self, paragraphs: List[Paragraph], title: Optional[Paragraph], level: int, rank: List[int], child_id: int):
    ...
    def create_children(self, paragraphs: List[Paragraph], level: int, rank: List[int]):
        ...
        child_id = 0
        while paragraphs:
            attached_paragraphs, container_paragraphs, in_children, current_level = self._classify_paragraphs(paragraphs)
            if container_paragraphs or self.title:
                children.append(self._create_child_container(container_paragraphs, self.title, current_level, rank, child_id))
                child_id += 1
            paragraphs = paragraphs[len(attached_paragraphs) + len(container_paragraphs):]
